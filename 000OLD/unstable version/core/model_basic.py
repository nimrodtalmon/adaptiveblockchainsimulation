...
def define_problem_7(instance):
    ...
    FEE2GAS_MAX = max([a["price"] for a in instance["apps"]])
...
def evaluate_utilities(app_assignments, op_assignments, fee2gas_chains, instance):
    ...
    # Qmax = \min(\sum_o gas_o, \sum_a gas_a) \cdot \max_a p_max(a)
    ...
    for a, c in enumerate(app_assignments):
        if c == -1 or demand[c] == 0:
            ...
            price_settled_in_chain = fee2gas_chains[c]
    ...
    for o, c in enumerate(op_assignments):
        if c == -1 or not ops_on_chain[c]:
            ...
            fee = fee2gas_chains[c] * gas[c] / Qmax_opsys

            op_util.append((fee * share) / ops[o]["stake"])

    # sys util
    sys_util = sum(fee2gas_chains[c] * gas[c] for c in chains) / Qmax_opsys
