...
def take_care_of_random_seed_for_opt():
...
def print_instance(instance, max_rows=10):
    ...
    for i, app in enumerate(instance["apps"][:max_rows]):
        print(f"  App {i}: gas={app['gas']:.1f}, stake={app['stake']:.1f}, price={app['price']:.1f}")
    ...
    for i, op in enumerate(instance["ops"][:max_rows]):
        print(f"  Op {i}: gas={op['gas']:.1f}, stake={op['stake']:.1f}, price={op['price']:.1f}")
...
# print constraint handler data of the base optimizaer employed
